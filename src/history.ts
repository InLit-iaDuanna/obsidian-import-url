...
export function normalizeRecentImports(entries: unknown): ImportHistoryEntry[] {
	...
	for (const entry of entries) {
		...
		normalized.push({
			...
			url: candidate.url,
			host: candidate.host,
			apiBaseUrl: typeof candidate.apiBaseUrl === "string" ? candidate.apiBaseUrl : "https://api.deepseek.com",
			...
			originalNotePath: typeof candidate.originalNotePath === "string" ? candidate.originalNotePath : undefined,
...
export function groupRecentImports(entries: ImportHistoryEntry[], now: Date = new Date()): ImportHistoryGroup[] {
	...
	const weekGroup: ImportHistoryGroup = {label: "最近 7 天", entries: []};
